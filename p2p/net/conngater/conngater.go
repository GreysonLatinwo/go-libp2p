...
func (cg *BasicConnectionGater) BlockPeer(p peer.ID) error {
	if cg.ds != nil {
		err := cg.ds.Put(context.TODO(), datastore.NewKey(keyPeer+p.String()), []byte(p))
		if err != nil {
...
func (cg *BasicConnectionGater) UnblockPeer(p peer.ID) error {
	if cg.ds != nil {
		err := cg.ds.Delete(context.TODO(), datastore.NewKey(keyPeer+p.String()))
		if err != nil {
...
func (cg *BasicConnectionGater) BlockAddr(ip net.IP) error {
	if cg.ds != nil {
		err := cg.ds.Put(context.TODO(), datastore.NewKey(keyAddr+ip.String()), []byte(ip))
		if err != nil {
...
func (cg *BasicConnectionGater) UnblockAddr(ip net.IP) error {
	if cg.ds != nil {
		err := cg.ds.Delete(context.TODO(), datastore.NewKey(keyAddr+ip.String()))
		if err != nil {
...
func (cg *BasicConnectionGater) BlockSubnet(ipnet *net.IPNet) error {
	if cg.ds != nil {
		err := cg.ds.Put(context.TODO(), datastore.NewKey(keySubnet+ipnet.String()), []byte(ipnet.String()))
		if err != nil {
			log.Errorf("error writing blocked addr to datastore: %s", err)
			return err
		}
...
func (cg *BasicConnectionGater) UnblockSubnet(ipnet *net.IPNet) error {
	if cg.ds != nil {
		err := cg.ds.Delete(context.TODO(), datastore.NewKey(keySubnet+ipnet.String()))
		if err != nil {
